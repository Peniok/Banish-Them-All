	...
	public class Menu : MonoBehaviour
	{
		...
		public void AnimateToTargetItem (MenuItem item)
		{
			...
			iTween.ValueTo (gameObject, iTween.Hash ("from", _currentMenuPosition, "to", _currentMenuPosition + offset, 
			                                         "time", 0.3, "easetype", iTween.EaseType.easeOutCubic, "onupdate", "UpdateCurrentMenuPosition"));
		}
		...
		//цих методів з самого початку не було, це я добавив
		public void Scenechooser(MenuItem item)
		{
			item.gameObject.GetComponent<SpriteRenderer>().color = new Color(0, 1, 0, 1);
		    FindObjectOfType<MainMenu>().NumberOfScene = int.Parse(item.gameObject.name);
		}
		public void WeaponChooser(MenuItem item)
		{
			item.gameObject.GetComponent<SpriteRenderer>().color = new Color(0, 1, 0, 1);
			PlayerPrefsSafe.SetInt("ChosedWeapon", int.Parse(item.gameObject.name));
			//NumberOfScene = int.Parse(item.gameObject.name);
		}
